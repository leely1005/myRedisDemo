...
public class MethodCacheInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
    	System.out.println("=======call method interceptor========");
        Object value = null;
...
//        String key = getCacheKey(targetName, methodName, arguments);
        String key = new StringBuffer("content:").append(arguments.length > 0 ? arguments[0].toString() : "").toString();
        try {
            // 判断是否有缓存
            if (exists(key)) {
                return getCache(key);
